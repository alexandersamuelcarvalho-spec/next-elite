...
function extractTeamRoles(row) {
  const teams = [];
  const colLetters = ['F','H','J','L','N','P','R','T','V'];
  for (const col of colLetters) {
    const team = row.get(col);
    const role = row.get(String.fromCharCode(col.charCodeAt(0) + 1));
    if (team) teams.push({ team, role });
  }
  return teams;
}
...
export async function updateMatch(leagueName, matchId, matchData) {
  ...
  if (row) {
    Object.entries(matchData).forEach(([key, val]) => row.set(key, val));
    // Auto-calculate winner/loser
    const homeScore = parseInt(matchData.homeScore || row.get('Home Score') || '0');
    const awayScore = parseInt(matchData.awayScore || row.get('Away Score') || '0');
...
async function updateLeagueTableStats(leagueName) {
  ...
  for (const teamRow of teamRows) {
    ...
    for (const match of matchRows) {
      ...
      const homeScore = parseInt(match.get('Home Score') || '0');
      const awayScore = parseInt(match.get('Away Score') || '0');
